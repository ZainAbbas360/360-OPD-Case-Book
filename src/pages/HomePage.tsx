...
type Palette = { wrap: string; chip: string; icon: string; glow: string };
const palettes: Record<string, Palette> = {
  Medicine: { wrap: 'from-med-50 via-white to-sky-50', chip: 'bg-med-100 text-med-800', icon: 'bg-med-600 text-white', glow: 'bg-med-200/50' },
  Surgery: { wrap: 'from-surg-50 via-white to-rose-50', chip: 'bg-surg-100 text-surg-800', icon: 'bg-surg-600 text-white', glow: 'bg-surg-200/50' },
  Pediatrics: { wrap: 'from-peds-50 via-white to-amber-50', chip: 'bg-peds-100 text-peds-800', icon: 'bg-peds-500 text-white', glow: 'bg-peds-200/50' },
  'Gynae/Obs': { wrap: 'from-purple-50 via-white to-pink-50', chip: 'bg-purple-100 text-purple-800', icon: 'bg-purple-600 text-white', glow: 'bg-purple-200/50' },
};
    ...
    <section className="relative overflow-hidden bg-gradient-to-br from-med-700 via-med-800 to-ink text-white">
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-20">
        <div className="max-w-2xl">
          ...
          <p className="mt-3 sm:mt-4 text-white/85 text-sm sm:text-lg">Browse by specialty, search instantly, and move case-to-case like a digital clinical handbook.</p>
          ...
          {Array.from(grouped.entries()).map(([specialty, items]) => {
            ...
            return <section key={specialty}>
              <div className="flex items-center justify-between gap-3 mb-4">
                <div className="flex items-center gap-3"><span className={`w-11 h-11 rounded-2xl flex items-center justify-center shadow-soft ${p.icon}`}><Icon className="w-5 h-5" /></span><div><h2 className="text-xl sm:text-2xl font-bold text-ink font-display">{specialty}</h2><p className="text-sm text-ink-muted">{items.length} cases · alphabetical within specialty</p></div></div>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">{items.map((c) => <CaseCard key={c.id} caseData={c} locked={!isPremium && !c.is_free} onNavigate={onNavigate} palette={p} />)}</div>
            </section>;
          })}
        </div>
      )}
